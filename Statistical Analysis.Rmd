...
df_complete <- read_csv("data/Complete_Data.csv", 
...
df_complete %>%
  select(subject, EMOTIONS, MUSICAL) %>%
  distinct() %>%
  summarise(mean_emo = mean(EMOTIONS),
            sd_emo = sd(EMOTIONS),
            mean_musical = mean(MUSICAL),
            sd_musical = sd(MUSICAL))

#---------------------------------------------------------------------------------------------------
# Main Effect of Intended Emotion

table_me_title <- expression(paste(italic("F"),"(3, 90) = 20.43, ", italic("p")," < .001"))
t1 <- expression(paste(italic("t"),"(126) = 5.95, ", italic("p"),"< .05 *"))
t2 <- expression(paste(italic("t"),"(126) = 7.09, ", italic("p"),"< .05 *"))
t3 <- expression(paste(italic("t"),"(126) = 3.92, ", italic("p"),"< .05 *"))
...
# 
# 
#       "list(~italic(t)(126)==5.95,~italic(p) < .05)",
#        "list(~italic(t)(126)==7.09,~italic(p) < .05)",
#        "list(~italic(t)(126)==3.92,~italic(p) < .05)"
# 
...
table_1 %>%
  ...
  geom_bracket(
    xmin = c("Happy", "Happy","Happy"), 
    xmax = c("Calm","Angry", "Sad"),
    y.position = c(8, 9, 10), 
    label = c(
      "list(~italic(t)(126)==5.95,~italic(p) < '.05')",
       "list(~italic(t)(126)==7.09,~italic(p) < '.05')",
...
table_valence_title <- expression(paste(italic("F"),"(1, 30) = 21.81, ", italic("p")," < .001"))
...
table_1 %>%
  ...
  coord_cartesian(ylim = c(1,9)) +
    ...
    xmax = c("Positive"),
    y.position = c(8), 
    label = c("list(~italic(p) < .001)"), 
    type = "expression",
    tip.length = 0.05) +
  labs(title = "Main effect of grouped analysis", subtitle = table_valence_title, y = "Mean", x = "Grouped analysis") +
...
table_lyrics_title <- expression(paste(italic("F"),"(1, 30) = 11.81, ", italic("p")," < .01"))

table_1 %>%
  group_by(lyrics) %>%
  summarise(mean_ratings = mean(means))  %>%
  mutate(lyrics = str_replace_all(string = lyrics, "Lyric","Lyrics")) %>%
  mutate(lyrics = str_replace_all(string = lyrics, "No Lyrics","No lyrics")) %>%
  ggplot(aes(y = mean_ratings, 
             x = lyrics)) +
  geom_bar(stat = "identity") +
  scale_y_continuous( breaks = seq(1,9,1)) +
  coord_cartesian(ylim = c(1,9)) +
    geom_bracket(
    xmin = c("Lyrics"), xmax = c("No lyrics"),
    y.position = c(8), label = c("list(~italic(p) < .001)"),
    type = "expression",
    tip.length = 0.05) +
  labs(title = "Main effect of lyrics", subtitle = table_lyrics_title, y = "Mean", x = "Presence of lyrics") +
...
ggsave(plot = cow_fig_1, height = 8, width = 9, dpi = 300, filename = "img/Figures/Figure1_labels.png")
ggsave(plot = cow_fig_1, height = 8, width = 9, dpi = 300, filename = "img/Figures/Figure1_labels.tiff")
ggsave(plot = cow_fig_1, height = 8, width = 9, dpi = 300, filename = "img/Figures/Figure1_labels.eps")
ggsave(plot = cow_fig_1, height = 8, width = 9, dpi = 300, filename = "img/Figures/Figure1_labels.jpeg")
...
effectsize::eta_squared(hypothesis_1_model, partial = TRUE)
...
summary(factorial_anova_direct_replication_valence)
...
df_replicate_anova %>%
  group_by(global_valence, Lyrics) %>%
  summarise(mean = mean(rating, na.rm = TRUE),
            sd = sd(rating, na.rm = TRUE)) %>%
  mutate(totz = paste(global_valence,Lyrics)) %>%
  ggplot(aes(x = totz, y = mean)) +
  geom_bar(stat = 'identity') 
...
bot_left_rep_title <- expression(paste(italic("F"), "(1, 103) = 203.46, ", italic("p")," > .001"))

"F (1, 103) = 203.46, p <.001"
...
bot_left_replication <- df_replicate_anova %>%
  ...
  geom_bracket(
    xmin = c("Lyrics"), 
    xmax = c("No lyrics"),
    y.position = c(7), label = c("list(~italic(p) < .001)"),
    type = "expression",
    tip.length = 0.05) +
    geom_errorbar(aes(ymin = response_mean + response_std, 
                      ymax = response_mean - response_std),  width = .25, size = .4) + 
  labs(title = "Main effect of lyrics",
       subtitle = bot_left_rep_title,
       x = "Lyrics", y = "Mean response")

bot_right_rep_title <- expression(paste(italic("F"), "(1, 103) = .04, ", italic("p")," > .05"))
...
bot_right_rep <- df_replicate_anova %>%
  ...
  geom_bracket(
    ...
    geom_errorbar(aes(ymin = response_mean + response_std, 
                      ymax = response_mean - response_std), width = .25, size = .4) + 
  labs(title = "Main effect of grouped analysis",
       subtitle = bot_right_rep_title,
       x = "Grouped analysis", y = "Mean response")


jg_title <- expression(paste(italic("F"),"(2.59, 264.30) = 28.24, ", italic("p")," <.001"))

top_replication_jg <- df_replicate_anova %>%
  ...
  ungroup(Emotion,Lyrics) %>%
  mutate(Lyrics = str_replace_all(string = Lyrics, pattern = "Melody","No lyrics")) %>%
  ggplot(aes(x = Lyrics, y = response_mean, linetype = Emotion, group = Emotion)) +
  geom_point() +
  geom_errorbar(aes(ymin = response_mean - response_std, ymax = response_mean + response_std), width = .1,  linetype = 1) +
  geom_line() +
  theme_minimal() +
  scale_y_continuous( breaks = seq(4,8,1), limits = c(4,8))  +
  labs(title = "Main effect and interaction of lyrics and emotion",
        subtitle = jg_title,
       x = "Condition",
       y = "Mean response")
...
# Assemble Figure 2
topper_rep <- plot_grid(top_replication_jg, labels = "A")
...
# Save Figure 2 
ggsave(plot = figure_2, height = 6, width = 7, dpi = 300, filename = "img/Figures/Figure2_labels.png")
ggsave(plot = figure_2, height = 6, width = 7, dpi = 300, filename = "img/Figures/Figure2_labels.tiff")
ggsave(plot = figure_2, height = 6, width = 7, dpi = 300, filename = "img/Figures/Figure2_labels.eps")
ggsave(plot = figure_2, height = 6, width = 7, dpi = 300, filename = "img/Figures/Figure2_labels.jpeg")
...
# Emo Cor Table 
cor.test(gmsi_table$MUSICAL, gmsi_table$EMOTIONS)

cor.test(sad_emotions$avg_emo, happy_emotions$avg_emo)

cor.test(calm_emotions$avg_emo, happy_emotions$avg_emo)
cor.test(angry_emotions$avg_emo, happy_emotions$avg_emo)

cor.test(sad_emotions$avg_emo, calm_emotions$avg_emo)
cor.test(sad_emotions$avg_emo, angry_emotions$avg_emo)
cor.test(calm_emotions$avg_emo, angry_emotions$avg_emo)
...
ggsave(filename = "img/Figures/Figure3_labels.eps", cor_plots_gmsi, units = "cm", dpi = 300, height = 10, width = 20)  
...
model_2_data <- model_2_data %>%
  rename(`Gold-MSI Musical` = MUSICAL,
         `Gold-MSI Emotions` = EMOTIONS,
         Response = response) %>%
  mutate(Condition = str_replace_all(Condition, pattern = "Melody", replacement = "No lyrics")) 

hypothesis_2_model <- lmerTest::lmer(rating ~ Condition*Response + `Gold-MSI Musical` + `Gold-MSI Emotions` + (1|subject), data = model_2_data)
...
hypothesis_2_model_noemo <- lmerTest::lmer(rating ~ Condition*response + MUSICAL + (1|subject), data = model_2_data)

summary(hypothesis_2_model_noemo)

sjPlot::tab_model(hypothesis_2_model_noemo)
```
