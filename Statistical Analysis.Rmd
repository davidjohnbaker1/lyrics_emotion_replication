...
table_1 %>%
  ...
  coord_cartesian(ylim = c(1,9)) +
    ...
    tip.length = 0.05) +
  labs(title = "Main Effect of Valence (Contrast)", subtitle = "F (1, 30) = 21.81, MSE = 1.68, p < .001", y = "Mean") +
...
#figure_1
grid.arrange(me_intended_emotion, foo, nrow = 2)
...
me_intended_emotion_lsu <- df_congruent %>%
  ...
  geom_bracket(
    xmin = c("Happy", "Happy","Happy","Calm","AngryStressful"), 
    xmax = c("Calm","AngryStressful","Sad","AngryStressful","Sad"),
...
grid.arrange(me_intended_emotion_lsu, conditions, nrow = 2)
...
angry_musical <- gmsi_cor(emotion_rating_table, gmsi_table, "AngryStressful", "MUSICAL")
...
angry_emotions <- gmsi_cor(emotion_rating_table, gmsi_table, "AngryStressful", "EMOTIONS")
...
emotion_rating_table %>%
  ...
  group_by(Emotion, response) %>%
# Make Plot 
  ggplot(aes(x = avg_rating, y = GMSI, color = Emotion)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_minimal() +
  facet_wrap(~response) +
  labs(title = "Average Rating Per Emotion vs GMSI", x = "Average Rating", y = "GMSI Score of Individual") -> cor_plots_gmsi

cor_plots_gmsi

#ggsave(filename = "img/cor_plots.png", cor_plots_gmsi, units = "cm", height = 10, width = 20)  
