...
master_data <- read_csv("Master_Data.csv", col_types = cols(age = col_character()))
...
master_data %>% 
  left_join(valence_table_exact) -> master_data
```
...
```{r}
long_master_data <- master_data %>%
  select(subject, Happy:AngryStressful, Condition, title, Valence) %>%
  pivot_longer(cols = Happy:AngryStressful, names_to = "response",values_to = "rating") %>%
  # Congrency Filter 
  mutate(congru = Valence == response) %>%
  filter(congru == TRUE)
...
anova_model <- lmer(rating ~ Condition*response + (1|subject), data = long_master_data)
anova(anova_model)
...
```{r}
# As Univariate 
collapsed_emotion_ratings <- master_data %>%
...
long_with_gmsi <- long_master_data %>%
  left_join(master_data %>%
              select(subject, GENERAL, EMOTIONS))


model_a <- lmer(rating ~ Condition:response + (1 | title), data = long_master_data)
summary(model_a)
0.04 / (0.04 + 6.96)
...
model_b <- lmer(rating ~ Condition:response + (1|title) + (1|subject), data = long_master_data)
summary(model_b)
